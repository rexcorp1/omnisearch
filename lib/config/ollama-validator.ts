 ...
 * Checks which models support 'tools' capability required for Omnisearch
    ...
    if (validated.size === 0) {
      console.error(
        '\n⚠️  ERROR: No Ollama models with tools support found!\n' +
          'Omnisearch requires models with tools capability for web search functionality.\n' +
          'Please install a model with tools support, for example:\n' +
          '  ollama pull qwen3\n' +
          '  ollama pull gpt-oss\n' +
          '  ollama pull deepseek-v3.1\n' +
          'Models without tools support will not work with Omnisearch.\n'
      )
    }
  } catch (error) {
    validationError = error as Error
    console.error('Ollama validation failed:', error)
    console.warn('Omnisearch will continue, but Ollama models may not work')
  }
}
...
export function validateOllamaModel(modelId: string): {
  ...
  if (!validatedModels!.has(modelId)) {
    return {
      valid: false,
      error: `Model "${modelId}" does not support tools capability required for Omnisearch. Please use a model with tools support.`
    }
  }
